...
trait MDT_Query
{
    ...
    function MDT_Query_method($model , $pathView  , $softDelete = false  , $data = []){
        ...
        $selectTrans      = array_key_exists('select_trans' , $data) ? $data['select_trans'] : null;
        ...
        if (is_array($selectTrans)) {

            $lang = app()->getLocale();

            $selectTransColumns =  [];

            foreach ($selectTrans as $key =>  $column):

                if ($column === "SKIP") {

                    $selectTransColumns[] =  $key;

                }else{

                    $selectTransColumns[] = $column."_$lang as $column";
                }
        ...
        if (\request()->ajax() && \request()->has('myDataTableAjax')) {
            ...
            isset($selectTransColumns) ? $modelDB->select($selectTransColumns) : '';
